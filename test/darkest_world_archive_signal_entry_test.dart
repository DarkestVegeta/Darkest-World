...
void main() {
  test('entry labels normalize underscores for archive presentation', () {
    final telemetry = DarkestWorldArchiveTelemetry.fromNavigation(
      DarkestWorldNavigationState(previous: null, current: _item('current'), next: null, related: const [], source: 'detail', entryPoint: 'archive_detail'),
    );
    expect(telemetry.entryPoint, 'ARCHIVE DETAIL');
    expect(telemetry.routeLabel, 'detail / ARCHIVE DETAIL');
  });
}
