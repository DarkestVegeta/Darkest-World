...
class _MapMode extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900), child: Padding(padding: const EdgeInsets.fromLTRB(30, 90, 30, 90), child: GridView.builder(shrinkWrap: true, itemCount: territories.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 2.2), itemBuilder: (_, i) {
    ...
    return InkWell(onTap: () => onSelect(i), child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: active ? const Color(0x332D2940) : const Color(0x180A0B12), border: Border.all(color: active ? const Color(0x617FFFFF) : const Color(0x1FFFFFFF))), child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(territories[i].name, style: TextStyle(color: active ? const Color(0xE6FFFFFF) : const Color(0x99FFFFFF), fontSize: 10, letterSpacing: 2)), const SizedBox(height: 5), Text(territories[i].description, style: const TextStyle(color: Color(0x4DFFFFFF), fontSize: 6))])), if (active) TextButton(onPressed: () => onEnter(i), child: const Text('ENTER'))]));
  }))));
}
