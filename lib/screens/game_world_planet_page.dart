...
class _GameWorldPlanetPageState extends State<GameWorldPlanetPage> with SingleTickerProviderStateMixin {
  ...
  static const territories = <Territory>[
    Territory('NINTENDO', 'Nintendo generations.', [
      GamePlatformGroup('HOME CONSOLES', 'Home generations.', [
        GamePlatform('NES', [18]), GamePlatform('SNES', [19]), GamePlatform('N64', [4]), GamePlatform('GameCube', [21]), GamePlatform('Wii', [5]), GamePlatform('Wii U', [41]), GamePlatform('Switch', [130]),
      ]),
      GamePlatformGroup('HANDHELD', 'Portable generations.', [
        GamePlatform('Game Boy', [33]), GamePlatform('Game Boy Color', [22]), GamePlatform('Game Boy Advance', [24]), GamePlatform('DS', [20]), GamePlatform('3DS', [37]),
      ]),
    ]),
    ...
    Territory('PLAYSTATION', 'PlayStation generations.', [
      GamePlatformGroup('GENERATIONS', 'Main generations.', [
        GamePlatform('PlayStation', [7]), GamePlatform('PlayStation 2', [8]), GamePlatform('PlayStation 3', [9]), GamePlatform('PlayStation 4', [48]), GamePlatform('PlayStation 5', [167]),
      ]),
    ]),
    Territory('XBOX', 'Xbox generations.', [
      GamePlatformGroup('GENERATIONS', 'Main generations.', [
  ...
  void enter(int index) => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GamePlatformPage(territory: territories[index].name, groups: territories[index].groups)));
          ...
          builder: (context, box) {
            ...
            return Stack(children: [
              ...
              SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 16), child: Row(children: [
                ...
                const Text('GAME-WORLD', style: TextStyle(fontSize: 15, letterSpacing: 4)),
                const Spacer(),
                Button(label: mapMode ? 'PLANET' : 'MAP', onTap: () => setState(() => mapMode = !mapMode)),
                const SizedBox(width: 6),
                Button(label: orbitOn ? 'ORBIT ON' : 'ORBIT OFF', onTap: toggleOrbit),
                const SizedBox(width: 6),
                Button(label: '−', onTap: () => setState(() => zoom = math.max(.82, zoom - .08))),
              ...
              if (mapMode)
                ...
                Center(child: SizedBox(width: side, height: side, child: Stack(children: [
                  ...
                  for (var i = 0; i < territories.length; i++)
                    Region(index: i, count: territories.length, name: territories[i].name, phase: controller.value, selected: selected == i, onTap: () => setState(() => selected = selected == i ? null : i)),
                ]))),
...
class Detail extends StatelessWidget {
  final Territory data; final VoidCallback close; final VoidCallback enter;
  const Detail({super.key, required this.data, required this.close, required this.enter});
  @override Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xEE080812), border: Border.all(color: const Color(0x2EFFFFFF))), child: Row(children: [
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(data.name, style: const TextStyle(fontSize: 13, letterSpacing: 3)), const SizedBox(height: 4), Text(data.description, style: const TextStyle(fontSize: 7, color: Color(0x61FFFFFF)))])),
    TextButton(onPressed: close, child: const Text('CLOSE')),
    FilledButton(onPressed: enter, child: const Text('ENTER')),
  ]));
}
...
class Region extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Align(alignment: Alignment(x * 2 - 1, y * 2 - 1), child: Opacity(opacity: opacity, child: GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: selected ? 90 : 68, height: selected ? 50 : 40, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0x33211D32), border: Border.all(color: selected ? const Color(0x99FFFFFF) : const Color(0x2EFFFFFF))), child: CustomPaint(painter: RegionPainter(index))),
      const SizedBox(height: 5),
      Text(name, style: TextStyle(color: selected ? Colors.white : const Color(0x8CFFFFFF), fontSize: selected ? 9 : 7, letterSpacing: 1.6)),
    ]))));
  }
}
...
class PlanetPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, radius, Paint()..shader = const RadialGradient(center: Alignment(-.28, -.32), radius: .9, colors: [Color(0xFF77718D), Color(0xFF29253A), Color(0xFF090A12), Color(0xFF020307)]).createShader(Rect.fromCircle(center: center, radius: radius)));
    final line = Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = const Color(0x223F4A65);
    for (var i = 0; i < 6; i++) canvas.drawOval(Rect.fromCenter(center: center, width: radius * (1.2 + i * .12), height: radius * (.22 + i * .035)), line);
    final grid = Paint()..style = PaintingStyle.stroke..strokeWidth = .45..color = const Color(0x1CFFFFFF);
    for (var i = -2; i <= 2; i++) canvas.drawArc(Rect.fromCircle(center: center, radius: radius * .93), -math.pi / 2 + i * .38 + phase * .08, math.pi, false, grid);
  }
...
class MapView extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900), child: GridView.builder(
      padding: const EdgeInsets.all(50), itemCount: territories.length,
      ...
      itemBuilder: (_, i) {
        ...
        return InkWell(onTap: () => onSelect(i), child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: active ? const Color(0x332D2940) : const Color(0x180A0B12), border: Border.all(color: active ? const Color(0x66FFFFFF) : const Color(0x1FFFFFFF))), child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(territories[i].name, style: TextStyle(color: active ? Colors.white : const Color(0x99FFFFFF), fontSize: 10, letterSpacing: 2)), const SizedBox(height: 5), Text(territories[i].description, style: const TextStyle(color: Color(0x4DFFFFFF), fontSize: 6))])),
          if (active) TextButton(onPressed: () => onEnter(i), child: const Text('ENTER')),
        ])));
      },
    ))));
  }
...
class SpacePainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    canvas.drawRect(Offset.zero & size, Paint()..shader = const RadialGradient(colors: [Color(0xFF19152B), Color(0xFF07070F), Color(0xFF010106)]).createShader(Offset.zero & size));
