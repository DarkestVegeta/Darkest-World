...
class _GameWorldPlanetPageState extends State<GameWorldPlanetPage> with SingleTickerProviderStateMixin {
  ...
  int? selected;
  bool autoOrbit = true, mapMode = false;
  double zoom = 1;
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF010207), body: AnimatedBuilder(animation: orbit, builder: (_, __) => LayoutBuilder(builder: (context, box) {
      ...
      return Stack(children: [
        ...
        if (mapMode)
          Positioned.fill(child: _MapMode(territories: territories, selected: selected, onSelect: (i) => setState(() => selected = i), onEnter: enter))
        else
        ...
        if (selected != null && !mapMode)
          Positioned(left: 22, right: 22, bottom: 20, child: _Detail(data: territories[selected!], onClose: () => setState(() => selected = null), onEnter: () => enter(selected!)))
        else if (!mapMode)
          const Positioned(left: 22, bottom: 20, child: _Legend()),
      ];
    })));
  }
}
...
class _PlanetRegion extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Align(alignment: Alignment(x * 2 - 1, y * 2 - 1), child: Opacity(opacity: visible ? 1 : .22, child: GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [
      AnimatedContainer(duration: const Duration(milliseconds: 180), width: selected ? 92 : 70, height: selected ? 52 : 42, decoration: BoxDecoration(shape: BoxShape.circle, color: selected ? const Color(0x443F385A) : const Color(0x33211D32), border: Border.all(color: selected ? const Color(0x8AFFFFFF) : const Color(0x2EFFFFFF)), boxShadow: selected ? const [BoxShadow(color: Color(0x554F4670), blurRadius: 24)] : const []), child: CustomPaint(painter: _RegionPainter(index))),
      const SizedBox(height: 5), Text(name, style: TextStyle(color: selected ? const Color(0xE6FFFFFF) : const Color(0x8CFFFFFF), fontSize: selected ? 9 : 7, letterSpacing: 1.7)),
    ]))));
  }
}
...
class _RegionPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    final p = Paint()..style = PaintingStyle.stroke..strokeWidth = .7..color = const Color(0x667D7591);
    final random = math.Random(seed + 17);
    for (var i = 0; i < 4; i++) canvas.drawOval(Rect.fromCenter(center: size.center(Offset.zero), width: size.width * (.35 + i * .15), height: size.height * (.22 + i * .12)), p);
    for (var i = 0; i < 9; i++) canvas.drawCircle(Offset(random.nextDouble() * size.width, random.nextDouble() * size.height), .45, p);
  }
...
class _MapMode extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900), child: Padding(padding: const EdgeInsets.fromLTRB(30, 90, 30, 90), child: GridView.builder(shrinkWrap: true, itemCount: territories.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 2.2), itemBuilder: (_, i) {
    ...
    return InkWell(onTap: () => onSelect(i), child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: active ? const Color(0x332D2940) : const Color(0x180A0B12), border: Border.all(color: active ? const Color(0x617FFFFFF) : const Color(0x1FFFFFFF))), child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(territories[i].name, style: TextStyle(color: active ? const Color(0xE6FFFFFF) : const Color(0x99FFFFFF), fontSize: 10, letterSpacing: 2)), const SizedBox(height: 5), Text(territories[i].description, style: const TextStyle(color: Color(0x4DFFFFFF), fontSize: 6))])), if (active) TextButton(onPressed: () => onEnter(i), child: const Text('ENTER'))]));
  }))));
}
...
class _SpacePainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 190; i++) { final x = (random.nextDouble() * size.width + t * size.width * .015) % size.width; final y = random.nextDouble() * size.height; canvas.drawCircle(Offset(x, y), .2 + random.nextDouble() * .55, Paint()..color = const Color(0x08FFFFFF)); }
  }
