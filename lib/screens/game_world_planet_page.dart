...
class _GameWorldPlanetPageState extends State<GameWorldPlanetPage> with SingleTickerProviderStateMixin {
  ...
  void enter(int index) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => GamePlatformPage(territory: territories[index].name, groups: territories[index].groups)));
  }

  void toggleOrbit() {
    setState(() => autoOrbit = !autoOrbit);
    if (autoOrbit) { orbit.repeat(); } else { orbit.stop(); }
  }
        ...
        builder: (_, __) => LayoutBuilder(builder: (context, box) {
          ...
          return Stack(children: [
            ...
            SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(22, 18, 22, 18), child: Row(children: [
              ...
              const Spacer(),
              _Control(label: mapMode ? 'PLANET' : 'MAP', onTap: () => setState(() => mapMode = !mapMode)),
              const SizedBox(width: 6),
              _Control(label: autoOrbit ? 'ORBIT ON' : 'ORBIT OFF', onTap: toggleOrbit),
              const SizedBox(width: 6),
              _Control(label: '−', onTap: () => setState(() => zoom = math.max(.82, zoom - .08))),
            ...
            Positioned(left: 22, bottom: 20, child: _Legend()),
...
class _PlanetRegion extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Align(alignment: Alignment(x * 2 - 1, y * 2 - 1), child: Opacity(opacity: visible ? 1 : .22, child: GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [AnimatedContainer(duration: const Duration(milliseconds: 180), width: selected ? 92 : 70, height: selected ? 52 : 42, decoration: BoxDecoration(shape: BoxShape.circle, color: selected ? const Color(0x443F385A) : const Color(0x33211D32), border: Border.all(color: selected ? Colors.white54 : Colors.white18), boxShadow: selected ? const [BoxShadow(color: Color(0x554F4670), blurRadius: 24)] : const []), child: CustomPaint(painter: _RegionPainter(index))), const SizedBox(height: 5), Text(name, style: TextStyle(color: Colors.white.withValues(alpha: selected ? .9 : .55), fontSize: selected ? 9 : 7, letterSpacing: 1.7))]))));
  }
}
...
class _PlanetPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    final c = size.center(Offset.zero); final r = size.shortestSide * .47;
...
class _MapMode extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900), child: Padding(padding: const EdgeInsets.fromLTRB(30, 90, 30, 90), child: GridView.builder(shrinkWrap: true, itemCount: territories.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 2.2), itemBuilder: (_, i) { final active = selected == i; return InkWell(onTap: () => onSelect(i), child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: active ? const Color(0x332D2940) : const Color(0x180A0B12), border: Border.all(color: active ? Colors.white38 : Colors.white12)), child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(territories[i].name, style: TextStyle(color: Colors.white.withValues(alpha: active ? .9 : .6), fontSize: 10, letterSpacing: 2)), const SizedBox(height: 5), Text(territories[i].description, style: const TextStyle(color: Colors.white30, fontSize: 6))])), if (active) TextButton(onPressed: () => onEnter(i), child: const Text('ENTER'))]))); }))));
}

class _SpacePainter extends CustomPainter {
  final double t; const _SpacePainter(this.t);
  @override void paint(Canvas canvas, Size size) { final random = math.Random(412); canvas.drawRect(Offset.zero & size, Paint()..shader = const RadialGradient(colors: [Color(0xFF19152B), Color(0xFF07070F), Color(0xFF010106)]).createShader(Offset.zero & size)); for (var i = 0; i < 190; i++) { final x = (random.nextDouble() * size.width + t * size.width * .015) % size.width; final y = random.nextDouble() * size.height; canvas.drawCircle(Offset(x, y), .2 + random.nextDouble() * .55, Paint()..color = Colors.white.withValues(alpha: .02)); } }
