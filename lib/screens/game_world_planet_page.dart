      ...
      body: LayoutBuilder(builder: (context, box) {
        ...
        return Stack(children: [
          ...
          Center(child: SizedBox(width: diameter, height: diameter, child: Stack(clipBehavior: Clip.none, children: [
            ...
            for (var i = 0; i < territories.length; i++) _TerritoryButton(
              data: territories[i], index: i, hovered: hovered == i, diameter: diameter,
              onEnter: () => setState(() => hovered = i), onExit: () => setState(() => hovered = null), onTap: () => _open(i),
            ),
          ...
          Positioned(left: compact ? 18 : 30, bottom: compact ? 18 : 26, child: Text('ENTER A TERRITORY', style: TextStyle(fontSize: 8, letterSpacing: 2.6, color: Colors.white.withValues(alpha: .22)))),
        ]);
      }),
    );
  }
...
class _TerritoryButton extends StatelessWidget {
  final _TerritoryData data; final int index; final bool hovered; final double diameter;
  final VoidCallback onEnter, onExit, onTap;
  const _TerritoryButton({required this.data, required this.index, required this.hovered, required this.diameter, required this.onEnter, required this.onExit, required this.onTap});
