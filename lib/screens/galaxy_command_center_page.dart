...
class _GalaxyCommandCenterPageState extends State<GalaxyCommandCenterPage> {
  ...
  List<GalaxyWorld> get _items {
    ...
    return widget.worlds.where((world) {
      final matchesText = q.isEmpty || '${world.title} ${world.description}'.toLowerCase().contains(q);
      final mapped = widget.visited.contains(world.kind);
      final matchesFilter = _filter == 'ALL' || (_filter == 'MAPPED' && mapped) || (_filter == 'UNMAPPED' && !mapped);
      return matchesText && matchesFilter;
    }).toList();
  }

  @override
  void initState() { super.initState(); _syncCursor(); }
  @override
  void dispose() { _search.dispose(); super.dispose(); }
  @override
  void didUpdateWidget(covariant GalaxyCommandCenterPage oldWidget) { super.didUpdateWidget(oldWidget); _syncCursor(); }

  void _syncCursor() {
    final items = _items;
    if (items.isEmpty) { _cursor = 0; return; }
  ...
  void _move(int delta) {
    ...
    setState(() { _cursor = (_cursor + delta) % items.length; if (_cursor < 0) _cursor = items.length - 1; });
  }

  void _openCursor() { final items = _items; if (items.isNotEmpty) widget.onOpen(items[_cursor.clamp(0, items.length - 1)]); }
      ...
      body: SafeArea(
        child: Focus(
          ...
          onKeyEvent: (_, event) {
            ...
            if (event.logicalKey == LogicalKeyboardKey.enter) { _openCursor(); return KeyEventResult.handled; }
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 5), _Filter(label: 'UNMAPPED', active: _filter == 'UNMAPPED', onTap: () => setState(() => _filter = 'UNMAPPED')),
              ]),
              ...
              Expanded(child: items.isEmpty
                  ...
                  : GridView.builder(
                      ...
                      itemBuilder: (_, index) {
                        ...
                        return Semantics(button: true, label: 'Command center gate ${world.title}', child: InkWell(
