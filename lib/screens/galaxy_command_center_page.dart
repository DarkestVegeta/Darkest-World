...
class _GalaxyCommandCenterPageState extends State<GalaxyCommandCenterPage> {
  ...
  List<GalaxyWorld> get items {
    ...
    return widget.worlds.where((w) {
      ...
      final filterOk = filter == 0 || (filter == 1 && mapped) || (filter == 2 && !mapped);
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Focus(autofocus: true, onKeyEvent: key, child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(
            controller: search,
            onChanged: (_) => setState(() => cursor = 0),
            style: const TextStyle(color: Colors.white70, fontSize: 9),
            decoration: const InputDecoration(
              hintText: 'SEARCH GATES', hintStyle: TextStyle(color: Color(0x33FFFFFF), fontSize: 7),
          ...
          Row(children: [
            ...
            FilterButton(label: 'MAPPED', active: filter == 1, onTap: () => setState(() { filter = 1; cursor = 0; })),
            const SizedBox(width: 6),
            FilterButton(label: 'UNMAPPED', active: filter == 2, onTap: () => setState(() { filter = 2; cursor = 0; })),
            const Spacer(), Text('${list.length} VISIBLE', style: const TextStyle(color: Color(0x33FFFFFF), fontSize: 6)),
          ]),
            ...
            child: list.isEmpty
                ...
                : GridView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return InkWell(
                        onTap: () => widget.onOpen(world),
                        child: Container(
                          padding: const EdgeInsets.all(14),
                          decoration: BoxDecoration(color: active ? const Color(0x221A1D2A) : const Color(0x120A0C14), border: Border.all(color: active ? const Color(0x66FFFFFF) : const Color(0x1AFFFFFF))),
                          child: Row(children: [
