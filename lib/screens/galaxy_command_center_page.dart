...
class _GalaxyCommandCenterPageState extends State<GalaxyCommandCenterPage> {
  ...
  List<GalaxyWorld> get items {
    ...
    return widget.worlds.where((world) {
      ...
      final filterOk = filter == 0 ||
          (filter == 1 && isMapped) ||
          (filter == 2 && !isMapped) ||
          (filter == 3 && isMapped && !isVisited) ||
          (filter == 4 && isVisited);
      final text = '${world.title} ${world.description}'.toLowerCase();
  ...
  void _syncCursor() {
    ...
    if (widget.selected != null) {
      final index = list.indexWhere((world) => world.kind == widget.selected);
      if (index >= 0) {
        cursor = index;
        return;
      }
  ...
  KeyEventResult handleKey(FocusNode node, KeyEvent event) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowLeft) {
      widget.onHistoryPrevious();
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
      widget.onHistoryNext();
      return KeyEventResult.handled;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final history = widget.routeHistory.length > 8
        ? widget.routeHistory.sublist(widget.routeHistory.length - 8)
        : widget.routeHistory;
            ...
            child: Column(
              ...
              children: [
                    ...
                    const Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 5),
                          Text('NAVIGATION / DISCOVERY / GATE CONTROL', style: TextStyle(color: Color(0x4DFFFFFF), fontSize: 7, letterSpacing: 1.5)),
                        ],
                      ),
                    ),
                    Column(
                ...
                SessionRoute(
                  ...
                  onPrevious: widget.onHistoryPrevious,
                  onNext: widget.onHistoryNext,
                ),
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          widget.visited.contains(current.kind)
                              ? 'VISITED'
                              : widget.mapped.contains(current.kind)
                                  ? 'MAPPED'
                                  : 'UNMAPPED',
                          style: const TextStyle(color: Color(0x66FFFFFF), fontSize: 5),
                        ),
                      ],
                    ),
                  ),
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintStyle: TextStyle(color: Color(0x33FFFFFF), fontSize: 7),
                    prefixIcon: Icon(Icons.search, color: Color(0x33FFFFFF), size: 15),
                    enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Color(0x1AFFFFFF))),
                    focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Color(0x40FFFFFF))),
                  ),
                ),
                const SizedBox(height: 10),
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ...
                      FilterButton(label: 'MAPPED', active: filter == 1, onTap: () => setState(() { filter = 1; _syncCursor(); })),
                      const SizedBox(width: 6),
                      FilterButton(label: 'UNMAPPED', active: filter == 2, onTap: () => setState(() { filter = 2; _syncCursor(); })),
                      const SizedBox(width: 6),
                      FilterButton(label: 'DISCOVERED', active: filter == 3, onTap: () => setState(() { filter = 3; _syncCursor(); })),
                      const SizedBox(width: 6),
                      FilterButton(label: 'VISITED', active: filter == 4, onTap: () => setState(() { filter = 4; _syncCursor(); })),
                      const SizedBox(width: 12),
                  ...
                  child: list.isEmpty
                      ...
                      : GridView.builder(
                          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                            ...
                            crossAxisSpacing: 9,
                            mainAxisSpacing: 9,
                            childAspectRatio: compact ? 3.2 : 2.3,
                          ),
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return InkWell(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: active ? const Color(0x221A1D2A) : const Color(0x120A0C14),
                                  border: Border.all(color: active ? const Color(0x66FFFFFF) : const Color(0x1AFFFFFF)),
                                ),
                                child: Row(
                                  children: [
                                    Text('${i + 1}'.padLeft(2, '0'), style: const TextStyle(color: Color(0x33FFFFFF), fontSize: 7)),
                                    const SizedBox(width: 12),
                                    Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        ...
                                        children: [
                                          Text(world.title, style: TextStyle(color: active ? Colors.white : const Color(0xA6FFFFFF), fontSize: 10, letterSpacing: 1.3)),
                                          const SizedBox(height: 5),
                                          Text(world.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0x47FFFFFF), fontSize: 6.2)),
                                        ],
                                      ),
                                    ),
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: history.asMap().entries.map((entry) {
                  final world = worlds.firstWhere((item) => item.kind == entry.value);
                  ...
                  return Padding(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: active ? const Color(0x18FFFFFF) : Colors.transparent,
                        border: Border.all(color: active ? const Color(0x45FFFFFF) : const Color(0x12FFFFFF)),
                      ),
                      child: Text('${entry.key + 1} ${world.title}', style: TextStyle(color: active ? Colors.white : const Color(0x55FFFFFF), fontSize: 5)),
                    ),
                  );
                }).toList(),
              ),
...
class RouteControls extends StatelessWidget {
  final GalaxyWorld? previous;
  final GalaxyWorld? current;
  final GalaxyWorld? next;
  final VoidCallback onPrevious;
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: const Border(right: BorderSide(color: Color(0x16FFFFFF))),
          ),
          child: Row(
            children: [
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active ? const Color(0x3DFFFFFF) : const Color(0x14FFFFFF)),
        ),
        child: Text(label, style: TextStyle(color: active ? const Color(0x99FFFFFF) : const Color(0x3DFFFFFF), fontSize: 5.5, letterSpacing: 1)),
      ),
    );
  }
}
