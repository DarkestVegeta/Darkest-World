...
class _GalaxyCommandCenterPageState extends State<GalaxyCommandCenterPage> {
  ...
  GalaxyWorld? worldFor(GalaxyWorldKind kind) => widget.worlds.where((w) => w.kind == kind).cast<GalaxyWorld?>().firstWhere((w) => w != null, orElse: () => null);
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Focus(autofocus: true, onKeyEvent: key, child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('GALAXY COMMAND CENTER', style: TextStyle(color: Colors.white, fontSize: 18, letterSpacing: 4)), SizedBox(height: 5), Text('NAVIGATION / DISCOVERY / GATE CONTROL', style: TextStyle(color: Color(0x4DFFFFFF), fontSize: 7, letterSpacing: 1.5))])),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('${widget.mapped.length}/${widget.worlds.length} MAPPED', style: const TextStyle(color: Color(0x59FFFFFF), fontSize: 7)), Text('${widget.visited.length} VISITED / ${widget.routeHistory.length} ROUTE EVENTS', style: const TextStyle(color: Color(0x38FFFFFF), fontSize: 5))]),
            const SizedBox(width: 12), TextButton(onPressed: widget.onClose, child: const Text('CLOSE')),
          ]),
          const SizedBox(height: 12),
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: const Color(0x080FFFFFF), border: Border.all(color: const Color(0x1AFFFFFF))), child: Row(children: [
            const Text('SESSION ROUTE', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 5, letterSpacing: 1.3)), const SizedBox(width: 10),
            Expanded(child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: history.asMap().entries.map((entry) { final world = worldFor(entry.value); final active = entry.value == widget.selected; return Padding(padding: const EdgeInsets.only(right: 5), child: Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 5), decoration: BoxDecoration(color: active ? const Color(0x18FFFFFF) : Colors.transparent, border: Border.all(color: active ? const Color(0x45FFFFFF) : const Color(0x12FFFFFF))), child: Text('${entry.key + 1} ${world?.title ?? 'UNKNOWN'}', style: TextStyle(color: active ? Colors.white : const Color(0x55FFFFFF), fontSize: 5)))); }).toList())),
            IconButton(onPressed: widget.onHistoryPrevious, tooltip: 'Previous route state', icon: const Icon(Icons.chevron_left, size: 16, color: Color(0x88FFFFFF))),
            IconButton(onPressed: widget.onHistoryNext, tooltip: 'Next route state', icon: const Icon(Icons.chevron_right, size: 16, color: Color(0x88FFFFFF))),
          ])),
          const SizedBox(height: 10),
          if (current != null) Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 11), decoration: BoxDecoration(color: const Color(0x0CFFFFFF), border: Border.all(color: const Color(0x24FFFFFF))), child: Row(children: [const Text('TARGET LOCK', style: TextStyle(color: Color(0x55FFFFFF), fontSize: 5, letterSpacing: 1.4)), const SizedBox(width: 12), Expanded(child: Text(current.title, style: const TextStyle(color: Colors.white, fontSize: 10, letterSpacing: 1.5))), Text(widget.visited.contains(current.kind) ? 'VISITED' : widget.mapped.contains(current.kind) ? 'MAPPED' : 'UNMAPPED', style: const TextStyle(color: Color(0x66FFFFFF), fontSize: 5))])),
          const SizedBox(height: 10),
          ...
          const SizedBox(height: 10),
          SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [FilterButton(label: 'ALL', active: filter == 0, onTap: () => setState(() { filter = 0; _syncCursor(); })), const SizedBox(width: 6), FilterButton(label: 'MAPPED', active: filter == 1, onTap: () => setState(() { filter = 1; _syncCursor(); })), const SizedBox(width: 6), FilterButton(label: 'UNMAPPED', active: filter == 2, onTap: () => setState(() { filter = 2; _syncCursor(); })), const SizedBox(width: 6), FilterButton(label: 'DISCOVERED', active: filter == 3, onTap: () => setState(() { filter = 3; _syncCursor(); })), const SizedBox(width: 6), FilterButton(label: 'VISITED', active: filter == 4, onTap: () => setState(() { filter = 4; _syncCursor(); })), const SizedBox(width: 12), Text('${list.length} VISIBLE', style: const TextStyle(color: Color(0x33FFFFFF), fontSize: 6))])),
          const SizedBox(height: 12),
          Expanded(child: list.isEmpty ? const Center(child: Text('NO GATES MATCH', style: TextStyle(color: Color(0x40FFFFFF), fontSize: 8, letterSpacing: 1.5))) : GridView.builder(
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: compact ? 1 : 2, crossAxisSpacing: 9, mainAxisSpacing: 9, childAspectRatio: compact ? 3.2 : 2.3), itemCount: list.length,
            itemBuilder: (_, i) { final world = list[i]; final active = i == cursor; final isMapped = widget.mapped.contains(world.kind); final isVisited = widget.visited.contains(world.kind); final status = isVisited ? 'VISITED / MAPPED' : isMapped ? 'MAPPED / UNVISITED' : 'UNMAPPED'; return InkWell(onTap: () { setState(() => cursor = i); widget.onOpen(world); }, child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: active ? const Color(0x221A1D2A) : const Color(0x120A0C14), border: Border.all(color: active ? const Color(0x66FFFFFF) : const Color(0x1AFFFFFF))), child: Row(children: [Text('${i + 1}'.padLeft(2, '0'), style: const TextStyle(color: Color(0x33FFFFFF), fontSize: 7)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(world.title, style: TextStyle(color: active ? Colors.white : const Color(0xA6FFFFFF), fontSize: 10, letterSpacing: 1.3)), const SizedBox(height: 5), Text(world.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0x47FFFFFF), fontSize: 6.2))])), Text(status, textAlign: TextAlign.right, style: TextStyle(color: isVisited ? const Color(0x70FFFFFF) : isMapped ? const Color(0x58FFFFFF) : const Color(0x30FFFFFF), fontSize: 5))]))); },
          )),
