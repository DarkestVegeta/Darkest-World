...
class _ContentDetailPageState extends State<ContentDetailPage> with SingleTickerProviderStateMixin {
  ...
  String? get _artUrl {
    final candidates = [widget.item.metadata['public_url'], widget.item.metadata['image_url'], widget.item.metadata['artwork_url'], widget.item.metadata['cover_url']];
  ...
  @override void initState() {
    super.initState();
    if (_isExternal) { _relatedLoading = false; _navigationLoading = false; }
    else { _loadRelated(); _loadNavigation(); }
  }
  @override void dispose() { _clock.dispose(); super.dispose(); }

  Future<void> _loadRelated() async {
    try { final result = await _repository.getRelatedContentItems(widget.item.id); if (!mounted) return; setState(() { _related = result; _relatedLoading = false; }); }
    catch (e) { if (!mounted) return; setState(() { _relatedLoading = false; _relatedError = e.toString(); }); }
  }
  Future<void> _loadNavigation() async {
    try { final result = await _repository.getTypedFranchiseNavigation(widget.item); if (!mounted) return; setState(() { _navigation = result; _navigationLoading = false; }); }
    catch (e) { if (!mounted) return; setState(() { _navigationLoading = false; _navigationError = e.toString(); }); }
  }
      ...
      body: AnimatedBuilder(
        ...
        builder: (_, __) => Stack(children: [
          ...
          SafeArea(child: ListView(padding: EdgeInsets.fromLTRB(compact ? 14 : 42, 14, compact ? 14 : 42, 55), children: [
            ...
            _WorldHero(item: widget.item, artUrl: _artUrl, compact: compact, onOpenChat: _chatContext == null ? null : () {},),
            const SizedBox(height: 22),
            ...
            else if (_navigationError != null) _MessagePanel(text: 'FRANCHISE NAVIGATION UNAVAILABLE')
...
class _WorldHero extends StatelessWidget {
  final ContentItem item; final String? artUrl; final bool compact; final VoidCallback? onOpenChat;
  const _WorldHero({required this.item, required this.artUrl, required this.compact, required this.onOpenChat});
  @override Widget build(BuildContext context) {
    final h = compact ? 390.0 : 470.0;
    return Container(height: h, decoration: BoxDecoration(color: const Color(0xCC070711), border: Border.all(color: const Color(0x507F70B0)), boxShadow: const [BoxShadow(color: Colors.black87, blurRadius: 35)]), child: compact
      ? Column(children: [Expanded(child: _Art(url: artUrl, title: item.title)), _HeroText(item: item, compact: true)])
      : Row(children: [Expanded(flex: 6, child: _Art(url: artUrl, title: item.title)), Expanded(flex: 5, child: _HeroText(item: item, compact: false))]));
  }
}
...
class _HeroText extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Padding(padding: EdgeInsets.all(compact ? 16 : 30), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
    const Text('CURRENT WORLD', style: TextStyle(fontSize: 7, letterSpacing: 3, color: Color(0x7897A8BE))),
    ...
    const Text('DARK CORE ARCHIVE', style: TextStyle(fontSize: 7, letterSpacing: 2, color: Color(0x55FFFFFF))),
    const SizedBox(height: 22),
    Row(children: [const Icon(Icons.public, size: 13, color: Color(0x778F82A9)), SizedBox(width: 7), Text(item.type.name.toUpperCase(), style: const TextStyle(fontSize: 7, letterSpacing: 1.8, color: Color(0x8897A8BE)))]),
  ]));
}

class _Art extends StatelessWidget {
  final String? url; final String title;
  const _Art({required this.url, required this.title});
  @override Widget build(BuildContext context) {
    if (url == null) return Container(decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF19152B), Color(0xFF06060C)])), child: Center(child: Text(title.toUpperCase(), textAlign: TextAlign.center, style: const TextStyle(fontSize: 13, letterSpacing: 2, color: Color(0x77FFFFFF)))));
    return Container(padding: const EdgeInsets.all(22), decoration: const BoxDecoration(gradient: RadialGradient(center: Alignment.center, radius: 1.1, colors: [Color(0xFF171328), Color(0xFF05050B)])), child: Image.network(url!, fit: BoxFit.contain, errorBuilder: (_, __, ___) => Center(child: Text(title, textAlign: TextAlign.center))));
  }
}
...
class _RelatedGrid extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => GridView.builder(physics: const NeverScrollableScrollPhysics(), shrinkWrap: true, itemCount: items.length, gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: compact ? 190 : 260, mainAxisExtent: compact ? 120 : 145, crossAxisSpacing: 10, mainAxisSpacing: 10), itemBuilder: (_, i) {
    ...
    return GestureDetector(onTap: () => open(item), child: Container(decoration: BoxDecoration(color: const Color(0x99080911), border: Border.all(color: const Color(0x267F70B0))), child: Row(children: [
      SizedBox(width: compact ? 65 : 82, height: double.infinity, child: url is String && url.isNotEmpty ? Image.network(url, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.public, color: Color(0x447F70B0))) : const Icon(Icons.public, color: Color(0x447F70B0))),
      Expanded(child: Padding(padding: const EdgeInsets.all(10), child: Text(item.title, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9, letterSpacing: .7, color: Color(0xCCFFFFFF))))),
    ])));
  });
}
...
class _DetailSpacePainter extends CustomPainter {
  ...
  @override void paint(Canvas c, Size s) {
    c.drawRect(Offset.zero & s, Paint()..shader = const RadialGradient(center: Alignment(0, -.15), radius: 1.2, colors: [Color(0xFF20192F), Color(0xFF090910), Color(0xFF010106)]).createShader(Offset.zero & s));
    final r = math.Random(42);
    for (var i = 0; i < 150; i++) { final x = r.nextDouble() * s.width; final y = r.nextDouble() * s.height; c.drawCircle(Offset(x, y), .35 + r.nextDouble(), Paint()..color = Colors.white.withValues(alpha: .06 + r.nextDouble() * .2)); }
    final y = (phase * s.height * 1.25) % (s.height + 120) - 60;
    c.drawRect(Rect.fromLTWH(0, y, s.width, 1), Paint()..color = const Color(0x127F70B0));
    c.drawRect(Offset.zero & s, Paint()..shader = const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0x00000000), Color(0x22000000), Color(0x99000000)]).createShader(Offset.zero & s));
  }
