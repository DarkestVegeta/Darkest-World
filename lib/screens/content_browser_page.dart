...
class _ContentBrowserPageState extends State<ContentBrowserPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> load() async {
    try {
      ...
      } else { result = await repository.getContentItemsPage(type: widget.contentType, page: 0); }
      if (mounted) setState(() { items.addAll(result); loading = false; });
    } catch (_) { if (mounted) setState(() => loading = false); }
  }
  ContentItem _snes(Map<String,dynamic> row) {
    final meta = row['metadata'] is Map ? Map<String,dynamic>.from(row['metadata']) : <String,dynamic>{}; meta['public_url'] = row['public_url'];
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: const Color(0xFF010107), body: AnimatedBuilder(animation: clock, builder: (_,__) => Stack(children:[
      Positioned.fill(child: CustomPaint(painter:_ArchivePainter(clock.value))),
      SafeArea(child: Padding(padding: EdgeInsets.fromLTRB(compact?14:30,compact?12:22,compact?14:30,12), child: Column(children:[
        ...
        Expanded(child: loading ? const Center(child:CircularProgressIndicator()) : shown.isEmpty ? const _EmptyArchive() : ListView(physics:const BouncingScrollPhysics(), children:[
          ...
          const SizedBox(height:22), _SectionHeader(count:shown.length, query:query), const SizedBox(height:10),
...
class _TopBar extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>Row(children:[
    ...
    Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title.toUpperCase(),style:const TextStyle(fontSize:12,letterSpacing:2.8)),const SizedBox(height:4),const Text('DARK CORE / CONTENT ARCHIVE',style:TextStyle(fontSize:6.5,letterSpacing:2,color:Color(0x55FFFFFF)))])),
    SizedBox(width:compact?150:260,height:34,child:TextField(onChanged:onSearch,style:const TextStyle(fontSize:10,color:Colors.white),decoration:InputDecoration(hintText:'SEARCH ARCHIVE',hintStyle:const TextStyle(fontSize:8,color:Color(0x55FFFFFF)),prefixIcon:const Icon(Icons.search,size:14,color:Color(0x88FFFFFF)),filled:true,fillColor:const Color(0x660A0B14),border:const OutlineInputBorder(borderSide:BorderSide(color:Color(0x227F70B0))),enabledBorder:const OutlineInputBorder(borderSide:BorderSide(color:Color(0x227F70B0))))))
  ]);
}
...
class _HeroArchive extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){
    final current=items[selected]; final previous=selected>0?items[selected-1]:null; final next=selected+1<items.length?items[selected+1]:null;
    return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[
      Expanded(child:_Neighbor(item:previous,label:'PREVIOUS',compact:compact,onTap:previous==null?null:()=>onSelect(selected-1))),const SizedBox(width:10),
      Expanded(flex:2,child:_Current(item:current,compact:compact,onOpen:()=>onOpen(current))),const SizedBox(width:10),
      Expanded(child:_Neighbor(item:next,label:'NEXT',compact:compact,onTap:next==null?null:()=>onSelect(selected+1))),
    ]),const SizedBox(height:9),Row(children:[const Text('PREVIOUS  |  ',style:TextStyle(fontSize:6.5,letterSpacing:1.8,color:Color(0x45FFFFFF))),Text(current.title.toUpperCase(),style:const TextStyle(fontSize:6.5,letterSpacing:1.8,color:Color(0xAFFFFFFF))),const Text('  |  NEXT',style:TextStyle(fontSize:6.5,letterSpacing:1.8,color:Color(0x45FFFFFF))),const Spacer(),Text('${selected+1} / ${items.length}',style:const TextStyle(fontSize:6.5,letterSpacing:1.8,color:Color(0x55FFFFFF)))])]);
  }
}
...
class _Current extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){
    final url='${item.metadata['public_url']??item.metadata['image_url']??''}'; final h=compact?250.0:330.0;
    return GestureDetector(onTap:onOpen,child:Container(height:h,decoration:BoxDecoration(color:const Color(0xCC080914),border:Border.all(color:const Color(0x657F70B0)),boxShadow:const[BoxShadow(color:Colors.black87,blurRadius:30)]),child:Row(children:[
      Expanded(flex:5,child:_Artwork(url:url,title:item.title)),Expanded(flex:6,child:Padding(padding:const EdgeInsets.all(20),child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisAlignment:MainAxisAlignment.center,children:[
        const Text('CURRENT',style:TextStyle(fontSize:7,letterSpacing:3,color:Color(0x7897A8BE))),const SizedBox(height:10),Text(item.title.toUpperCase(),maxLines:3,overflow:TextOverflow.ellipsis,style:TextStyle(fontSize:compact?17:25,height:1.05,letterSpacing:2.5,fontWeight:FontWeight.w300)),const SizedBox(height:13),const Text('DARK CORE ARCHIVE',style:TextStyle(fontSize:7,letterSpacing:2,color:Color(0x66FFFFFF))),const SizedBox(height:22),FilledButton(onPressed:onOpen,child:const Text('OPEN WORLD')),
      ]))),
    ])));
  }
}
...
class _Neighbor extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){final url=item==null?'':'${item!.metadata['public_url']??item!.metadata['image_url']??''}';return GestureDetector(onTap:onTap,child:Opacity(opacity:item==null?.22:.75,child:Container(height:compact?250:330,decoration:BoxDecoration(color:const Color(0x88070810),border:Border.all(color:const Color(0x267F70B0))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Expanded(child:_Artwork(url:url,title:item?.title??'—',muted:true)),Padding(padding:const EdgeInsets.fromLTRB(12,9,12,12),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(label,style:const TextStyle(fontSize:6,letterSpacing:2,color:Color(0x5597A8BE))),const SizedBox(height:5),Text(item?.title.toUpperCase()??'—',maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:8.5,letterSpacing:1.1,color:Color(0xBBFFFFFF)))]))]))));}
}
...
class _ArchiveGrid extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>GridView.builder(physics:const NeverScrollableScrollPhysics(),shrinkWrap:true,itemCount:items.length,gridDelegate:SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent:compact?170:220,mainAxisExtent:compact?245:285,crossAxisSpacing:12,mainAxisSpacing:12),itemBuilder:(_,i){final item=items[i];final url='${item.metadata['public_url']??item.metadata['image_url']??''}';final active=i==selected;return GestureDetector(onTap:()=>onSelect(i),onDoubleTap:()=>onOpen(item),child:AnimatedContainer(duration:const Duration(milliseconds:220),decoration:BoxDecoration(color:const Color(0xCC080811),border:Border.all(color:active?const Color(0xAA8A78B5):const Color(0x1DFFFFFF)),boxShadow:active?const[BoxShadow(color:Color(0x401F163B),blurRadius:22)]:null),child:Column(children:[Expanded(child:_Artwork(url:url,title:item.title)),Container(padding:const EdgeInsets.fromLTRB(10,9,10,11),width:double.infinity,child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('${(i+1).toString().padLeft(3,'0')}  /  ARCHIVE',style:const TextStyle(fontSize:5.5,letterSpacing:1.6,color:Color(0x4FFFFFFF))),const SizedBox(height:5),Text(item.title,maxLines:2,overflow:TextOverflow.ellipsis,style:TextStyle(fontSize:9,letterSpacing:.7,color:active?Colors.white:const Color(0xBBFFFFFF)))]))])));});
}

class _Artwork extends StatelessWidget { final String url; final String title; final bool muted; const _Artwork({required this.url,required this.title,this.muted=false}); @override Widget build(BuildContext context)=>url.isEmpty?Container(decoration:const BoxDecoration(gradient:LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF151426),Color(0xFF07070D)])),child:Center(child:Padding(padding:const EdgeInsets.all(15),child:Text(title,textAlign:TextAlign.center,style:TextStyle(fontSize:11,letterSpacing:1.2,color:muted?const Color(0x77FFFFFF):Colors.white))))):Image.network(url,fit:BoxFit.cover,width:double.infinity,errorBuilder:(_,__,___)=>Center(child:Text(title,textAlign:TextAlign.center)));
}

class _SectionHeader extends StatelessWidget { final int count; final String query; const _SectionHeader({required this.count,required this.query}); @override Widget build(BuildContext context)=>Row(children:[const Text('ARCHIVE',style:TextStyle(fontSize:9,letterSpacing:3)),const SizedBox(width:10),Text('$count ITEMS',style:const TextStyle(fontSize:6.5,letterSpacing:1.7,color:Color(0x55FFFFFF))),if(query.isNotEmpty)...[const SizedBox(width:10),Text('FILTER: $query',style:const TextStyle(fontSize:6.5,letterSpacing:1.2,color:Color(0x668D7DB2)))]]); }
