    ...
    return Scaffold(backgroundColor: const Color(0xFF010207), body: Stack(children: [
      ...
      SafeArea(child: Stack(children: [
        ...
        Positioned.fill(child: Center(child: LayoutBuilder(builder: (context, constraints) {
          ...
          return SizedBox(width: size, height: size, child: GestureDetector(
            onTapUp: (details) => _selectFromPoint(details.localPosition, size),
            child: CustomPaint(painter: _FamilyPlanetPainter(selected: selected), foregroundPainter: _FamilyMemberPainter(members.length, selected: selected)),
          ));
        }))),
...
class _SelectionPanel extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    width: 370, padding: const EdgeInsets.all(18),
    decoration: BoxDecoration(color: const Color(0xFF070A12).withValues(alpha: .94), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFF829BC4).withValues(alpha: .22)), boxShadow: const [BoxShadow(color: Colors.black54, blurRadius: 24)]),
    child: Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(member.name, style: const TextStyle(fontSize: 14, letterSpacing: 2, fontWeight: FontWeight.w600)), const SizedBox(height: 7),
        Text(member.description, style: TextStyle(color: Colors.white.withValues(alpha: .55), fontSize: 12, height: 1.4)), const SizedBox(height: 12),
...
class _FamilySpacePainter extends CustomPainter {
  @override void paint(Canvas canvas, Size size) {
    ...
    final random = math.Random(84), starPaint = Paint()..color = Colors.white.withValues(alpha: .22);
    for (var i = 0; i < 240; i++) { final p = Offset(random.nextDouble() * size.width, random.nextDouble() * size.height); canvas.drawCircle(p, .35 + random.nextDouble() * 1.1, starPaint); }
  }
...
class _FamilyPlanetPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    final center = Offset(size.width / 2, size.height / 2), radius = size.width * .46;
    ...
    for (var i = 0; i < 15; i++) { final a = i * .83, p = Offset(center.dx + math.cos(a) * radius * .45, center.dy + math.sin(a) * radius * .37); canvas.drawOval(Rect.fromCenter(center: p, width: radius * (.14 + (i % 3) * .09), height: radius * (.07 + (i % 2) * .05)), terrain); }
...
class _FamilyMemberPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    final center = Offset(size.width / 2, size.height / 2), radius = size.width * .36;
    for (var i = 0; i < count; i++) { final angle = i * (math.pi * 2 / count) + math.pi / 8, p = Offset(center.dx + math.cos(angle) * radius, center.dy + math.sin(angle) * radius), active = selected == i; final paint = Paint()..color = (active ? const Color(0xFFC2D0E8) : Colors.white).withValues(alpha: active ? .9 : .3); canvas.drawCircle(p, active ? 6 : 3.5, paint); }
  }
