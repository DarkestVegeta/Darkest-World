...
class _GamePlatformPageState extends State<GamePlatformPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: LayoutBuilder(builder: (context, box) {
        ...
        final width = math.min(box.maxWidth * .94, 1280.0);
        final height = math.min(box.maxHeight * .78, 720.0);
        ...
        return Stack(children: [
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: const RadialGradient(colors: [Color(0xFF302A45), Color(0xFF0B0A12), Color(0xFF020208)]),
                    border: Border.all(color: Colors.white.withValues(alpha: .14)),
                  ),
                  child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Text(widget.territory, style: const TextStyle(fontSize: 10, letterSpacing: 2)),
                    const SizedBox(height: 5),
                    Text('WORLD', style: TextStyle(fontSize: 7, letterSpacing: 2.4, color: Colors.white.withValues(alpha: .28))),
                  ])),
                ),
              ),
            ]),
          ...
          Positioned(
            ...
            child: Text(
              selected == null ? 'SELECT PLATFORM' : '${platforms[selected!].name.toUpperCase()}  •  ENTER WORLD',
              textAlign: TextAlign.center,
              style: TextStyle(fontSize: 8, letterSpacing: 2.4, color: Colors.white.withValues(alpha: .30)),
            ),
          ),
        ]);
      }),
  ...
  List<Offset> _positions(Offset center, double width, double height, int count) {
    return List.generate(count, (i) {
      ...
      final rx = width * (.18 + (i % 3) * .09);
      final ry = height * (.16 + (i % 3) * .07);
      return Offset(center.dx + math.cos(angle) * rx, center.dy + math.sin(angle) * ry);
    });
  }
}
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            gradient: const RadialGradient(colors: [Color(0xFF6B608A), Color(0xFF242136), Color(0xFF05060C)]),
            border: Border.all(color: Colors.white.withValues(alpha: active ? .28 : .09), width: active ? 1.4 : .7),
          ),
          child: Center(child: Text(platform.name, textAlign: TextAlign.center, maxLines: 2, style: TextStyle(fontSize: active ? 10 : 8, letterSpacing: 1.1, color: Colors.white.withValues(alpha: active ? .95 : .62)))),
        ),
      ),
    );
  }
...
class _PlatformBackgroundPainter extends CustomPainter {
  ...
  @override void paint(Canvas c, Size s) {
    c.drawRect(Offset.zero & s, Paint()..shader = const RadialGradient(colors: [Color(0xFF17142A), Color(0xFF06060D), Color(0xFF010106)]).createShader(Offset.zero & s));
    final r = math.Random(412);
    final p = Paint();
    for (var i = 0; i < 260; i++) {
      p.color = Colors.white.withValues(alpha: .025 + r.nextDouble() * .10);
      c.drawCircle(Offset(r.nextDouble() * s.width, r.nextDouble() * s.height), .2 + r.nextDouble() * .7, p);
    }
  }
  @override bool shouldRepaint(covariant _PlatformBackgroundPainter old) => false;
}
...
class _RoutesPainter extends CustomPainter {
  ...
  @override void paint(Canvas c, Size s) {
    final p = Paint()..style = PaintingStyle.stroke..strokeWidth = .7..color = const Color(0x227F72A0);
    for (final point in positions) c.drawLine(center, point, p);
  }
  @override bool shouldRepaint(covariant _RoutesPainter old) => false;
}
