    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: const Color(0xFF080817).withValues(alpha: .82),
          border: Border.all(color: const Color(0xFF8170FF).withValues(alpha: .14)),
          boxShadow: [
            BoxShadow(
              color: const Color(0xFF5945C8).withValues(alpha: .07),
              blurRadius: 30,
            ),
          ],
        ),
        child: Column(
          ...
          children: [
            ...
            Text(
              group.subtitle,
              style: TextStyle(color: Colors.white.withValues(alpha: .35), fontSize: 10),
            ),
            const SizedBox(height: 14),
            Wrap(
              spacing: 9,
              runSpacing: 9,
              children: group.platforms.map((platform) {
                return _PlatformButton(platform: platform);
              }).toList(),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return InkWell(
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => ContentBrowserPage(
              title: '${platform.name} • GAME-WORLD',
              contentType: 'game',
              platformIds: platform.igdbIds,
            ),
          ),
        );
      },
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: const Color(0xFF8D7BFF).withValues(alpha: .16)),
          gradient: const LinearGradient(
            colors: [Color(0xFF11102A), Color(0xFF080812)],
          ),
        ),
        child: Row(
          ...
          children: [
            Container(
              width: 7,
              height: 7,
              decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0xFF8D7BFF)),
            ),
