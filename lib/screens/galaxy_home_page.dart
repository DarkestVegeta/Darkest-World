...
class _GalaxyHomePageState extends State<GalaxyHomePage> {
  ...
  void openWorld(GalaxyWorld world) {
    ...
    switch (world.kind) {
      case GalaxyWorldKind.vegeta:
        page = DarkCorePage(title: 'VEGETA WORLD', description: world.description);
      case GalaxyWorldKind.game:
        page = const GameWorldPlanetPage();
      case GalaxyWorldKind.music:
        page = MusicWorldPage(title: world.title, description: world.description);
      case GalaxyWorldKind.identity:
        page = IdentityWorldPage(title: world.title, description: world.description);
      case GalaxyWorldKind.family:
        page = FamilyWorldPage(title: world.title, description: world.description);
      case GalaxyWorldKind.cinema:
        page = CinemaWorldPage(title: world.title, description: world.description);
      case GalaxyWorldKind.creation:
        page = const CreationWorldPage();
      case GalaxyWorldKind.archive:
        page = const ArchiveWorldPage();
      case GalaxyWorldKind.comingSoon:
        page = const ComingSoonWorldPage();
    }
  ...
  void historyMove(int delta) {
    ...
    if (kind != null) selectWorld(worldFor(kind));
  }
        ...
        child: Stack(
          ...
          children: [
            ...
            Positioned(
              left: 18,
              bottom: 18,
              child: Info(text: session.selected == null ? 'SCANNING / SELECT A WORLD' : 'TARGET ${session.selected.toString().split('.').last.toUpperCase()} / LOCKED'),
            ),
            Positioned(
              right: 18,
              bottom: 18,
              child: Info(text: 'VISITS ${session.visits} / MAPPED ${session.mapped.length}/${worlds.length} / VISITED ${session.visited.length} / ROUTE ${session.routeHistory.length}'),
            ),
            if (session.selected != null)
              Positioned(
                left: 18,
                ...
                child: RouteBar(
                  ...
                  onNext: () => routeMove(1),
                  onCurrent: () => openWorld(worldFor(session.selected!)),
                ),
              ),
            if (atlas)
              Atlas(
                ...
                selected: session.selected,
                discoveryOrder: session.discoveryOrder,
                gateHistory: session.gateHistory,
                close: () => setState(() => atlas = false),
                open: openWorld,
                revisit: revisitLastGate,
              ),
            if (command)
              Positioned.fill(
                child: Material(
  ...
  Widget build(BuildContext context) => Expanded(
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(border: const Border(right: BorderSide(color: Color(0x1AFFFFFF))), color: active ? const Color(0x0FFFFFFF) : Colors.transparent),
            child: Row(
              children: [
...
class Atlas extends StatelessWidget {
  ...
  final List<GalaxyWorldKind> gateHistory;
  final VoidCallback close;
  final ValueChanged<GalaxyWorld> open;
  final VoidCallback revisit;
