...
class _ArchiveStagePainter extends CustomPainter {
  ...
  @override void paint(Canvas c, Size s) {
    ...
    c.drawRect(Offset.zero & s, Paint()..shader = RadialGradient(center: const Alignment(0,0), radius: 1.15, colors: [const Color(0x160B0B1B), Colors.transparent, const Color(0x39000000)]).createShader(Offset.zero & s));
    for (var i=0;i<140;i++) { final p=Offset(rng.nextDouble()*s.width,rng.nextDouble()*s.height); c.drawCircle(p,.25+rng.nextDouble()*.75,Paint()..color=Colors.white.withValues(alpha:.025+rng.nextDouble()*.07)); }
    for (var i=0;i<4;i++) { final rr=r*(1.42+i*.23); c.drawOval(Rect.fromCenter(center:center,width:rr*2.25,height:rr*.62),Paint()..style=PaintingStyle.stroke..strokeWidth=.55..color=const Color(0x18A18DBB)); }
    ...
    for(var i=0;i<12;i++) { final y=center.dy-r*.75+i*r*.13+math.sin(phase*math.pi*2+i)*r*.015; c.drawLine(Offset(center.dx-r,y),Offset(center.dx+r,y),Paint()..strokeWidth=.6..color=const Color(0x287F718F)); }
    for(var i=0;i<18;i++) { final a=i*math.pi*2/18+phase*.045; final p=Offset(center.dx+math.cos(a)*r*.72,center.dy+math.sin(a)*r*.72); c.drawCircle(p,r*(.012+(i%3)*.006),Paint()..color=const Color(0x4AABA0BA)); }
    c.restore();
    ...
    final label=title.toUpperCase(); final tp=TextPainter(text:TextSpan(text:label,style:TextStyle(color:Colors.white.withValues(alpha:.72),fontSize:compact?8:10,letterSpacing:3,fontWeight:FontWeight.w300)),textDirection:TextDirection.ltr)..layout(maxWidth:s.width*.42); tp.paint(c,Offset(center.dx-tp.width/2,center.dy+r+18));
    final meta=TextPainter(text:const TextSpan(text:'ARCHIVE CORE  /  LIVE SELECTION',style:TextStyle(color:Color(0x55FFFFFF),fontSize:6,letterSpacing:2)),textDirection:TextDirection.ltr)..layout(); meta.paint(c,Offset(center.dx-meta.width/2,center.dy+r+36));
