...
class _DarkestWorldUniverseState extends State<DarkestWorldUniverse> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    ...
    return FocusableActionDetector(
      autofocus: true,
      shortcuts: const {SingleActivator(LogicalKeyboardKey.enter): ActivateIntent(), SingleActivator(LogicalKeyboardKey.escape): DismissIntent()},
      actions: {ActivateIntent: CallbackAction<ActivateIntent>(onInvoke: (_) { visit(); return null; }), DismissIntent: CallbackAction<DismissIntent>(onInvoke: (_) { setState(() => selected = null); return null; })},
      child: Scaffold(backgroundColor: const Color(0xFF010107), body: GestureDetector(
        onScaleUpdate: (d) => setState(() { if (d.pointerCount > 1) { zoom = (zoom * d.scale).clamp(.66, 1.70).toDouble(); } else { orbit += d.focalPointDelta.dx / math.max(220.0, size.width); } }),
        child: AnimatedBuilder(animation: clock, builder: (_, __) => Stack(fit: StackFit.expand, children: [
          ...
          Transform.scale(scale: zoom, child: CustomPaint(painter: _OrbitArchitecturePainter(clock.value, orbit, systemMap, detail))),
          _WorldOrbit(worlds: widget.worlds, phase: clock.value, orbit: orbit, selected: selected, hovered: hovered, labels: labels, detail: detail, cinematic: cinematic, compact: compact, onTap: (w) => setState(() => selected = selected == w.kind ? null : w.kind), onHover: (w) => setState(() => hovered = w?.kind)),
          _Header(systemMap: systemMap, compact: compact, cinematic: cinematic),
          Positioned(right: compact ? 10 : 26, top: compact ? 70 : 26, child: _Controls(map: systemMap, labels: labels, detail: detail, cinematic: cinematic, onIn: () => setState(() => zoom = (zoom + .1).clamp(.66, 1.70).toDouble()), onOut: () => setState(() => zoom = (zoom - .1).clamp(.66, 1.70).toDouble()), onMap: () => setState(() => systemMap = !systemMap), onLabels: () => setState(() => labels = !labels), onDetail: () => setState(() => detail = !detail), onCinematic: () => setState(() => cinematic = !cinematic), onReset: () => setState(() { orbit = 0; zoom = 1; selected = null; hovered = null; systemMap = false; detail = true; cinematic = true; }))),
          ...
          if (chosen != null) _FloatingVisitPanel(world: chosen, compact: compact, orbit: orbit, count: widget.worlds.length, index: widget.worlds.indexOf(chosen), onVisit: visit, onClose: () => setState(() => selected = null)) else const Positioned(left: 0, right: 0, bottom: 22, child: Center(child: _Hint())),
        ])),
      )),
    );
  }
...
class _Controls extends StatelessWidget {
  final bool map, labels, detail, cinematic; final VoidCallback onIn, onOut, onMap, onLabels, onDetail, onCinematic, onReset;
  ...
  @override Widget build(BuildContext context) => Wrap(spacing: 4, children: [_Btn('+', onIn), _Btn('−', onOut), _Btn(map ? 'ORBIT' : 'MAP', onMap), _Btn(labels ? 'LABELS' : 'CLEAN', onLabels), _Btn(detail ? 'DETAIL' : 'MINIMAL', onDetail), _Btn(cinematic ? 'CINEMATIC' : 'EFFICIENT', onCinematic), _Btn('RESET', onReset)]);
}
...
class _OrbitArchitecturePainter extends CustomPainter {
  ...
  @override void paint(Canvas c, Size s) { final center = Offset(s.width * .52, s.height * .52), m = math.min(s.width, s.height), p = Paint()..style = PaintingStyle.stroke; for (var i = 0; i < 16; i++) { final r = m * (.12 + i * .03); p.color = Colors.white.withValues(alpha: map ? .045 : .022 + (i % 3) * .006); p.strokeWidth = i == 7 ? 1.1 : .42; c.drawOval(Rect.fromCenter(center: center, width: r * 2.4, height: r * (.31 + (i % 5) * .065) * 2), p); } if (detail) for (var i = 0; i < 13; i++) { final r = m * (.15 + i * .035); p.color = Colors.white.withValues(alpha: map ? .025 : .014); p.strokeWidth = .34; c.drawOval(Rect.fromCenter(center: center, width: r * 2.7, height: r * .28), p); } }
  @override bool shouldRepaint(covariant _OrbitArchitecturePainter o) => o.phase != phase || o.orbit != orbit || o.map != map || o.detail != detail;
}

class _NodePainter extends CustomPainter {
  final GalaxyWorldKind kind; final double phase; final int index; final bool selected, hovered, labels, detail, cinematic; final String title; _NodePainter(this.kind, this.phase, this.index, this.selected, this.hovered, this.labels, this.detail, this.cinematic, this.title);
  @override void paint(Canvas c, Size s) { final center = Offset(s.width / 2, s.height / 2), r = s.width * .30, rect = Rect.fromCircle(center: center, radius: r); c.drawCircle(center, r * (hovered ? 1.95 : 1.62), Paint()..shader = RadialGradient(colors: [Colors.white.withValues(alpha: selected ? .24 : hovered ? .16 : .06), Colors.transparent]).createShader(rect.inflate(r))); c.drawCircle(center, r, Paint()..shader = RadialGradient(center: const Alignment(-.38, -.42), radius: 1.04, colors: [const Color(0xFFB5BFCA), _tone(kind), const Color(0xFF03040A)]).createShader(rect)); c.drawCircle(center, r, Paint()..shader = RadialGradient(center: const Alignment(.70, .62), radius: .92, colors: [Colors.transparent, Colors.black.withValues(alpha: .72)]).createShader(rect)); if (selected) c.drawCircle(center, r * 1.22, Paint()..color = Colors.white38..style = PaintingStyle.stroke..strokeWidth = 1); if (labels) { final tp = TextPainter(text: TextSpan(text: title.toUpperCase(), style: TextStyle(color: Colors.white.withValues(alpha: selected ? .82 : hovered ? .72 : .42), fontSize: math.max(6, s.width * .025), letterSpacing: 1.2)), textDirection: TextDirection.ltr)..layout(maxWidth: s.width * 2.2); tp.paint(c, Offset(center.dx - tp.width / 2, center.dy + r * 1.38)); } }
  @override bool shouldRepaint(covariant _NodePainter o) => o.kind != kind || o.phase != phase || o.selected != selected || o.hovered != hovered || o.labels != labels || o.detail != detail || o.cinematic != cinematic || o.title != title;
}
...
class _DeepSpacePainter extends CustomPainter {
  ...
  @override void paint(Canvas c, Size s) {
    ...
    final center = Offset(s.width * .50, s.height * .49); c.drawCircle(center, m * .86, Paint()..shader = RadialGradient(center: const Alignment(-.05, -.08), radius: 1, colors: [const Color(0xFF342049).withValues(alpha: .23), const Color(0xFF17264D).withValues(alpha: .12), const Color(0xFF090A18).withValues(alpha: .035), Colors.transparent], stops: const [0, .38, .68, 1]).createShader(Rect.fromCircle(center: center, radius: m * .86)));

    // ONE asymmetrical cosmic filament: a random slice through a much larger natural cloud.
    // It enters thin and uncertain, swells heavily off-centre, bends irregularly, then breaks down and disappears.
    final path = Path()..moveTo(-m * .18, s.height * .72);
    path.cubicTo(s.width * .01, s.height * .68, s.width * .08, s.height * .58, s.width * .20, s.height * .55);
    path.cubicTo(s.width * .29, s.height * .53, s.width * .31, s.height * .31, s.width * .43, s.height * .27);
    path.cubicTo(s.width * .56, s.height * .22, s.width * .56, s.height * .45, s.width * .68, s.height * .50);
    path.cubicTo(s.width * .77, s.height * .55, s.width * .81, s.height * .43, s.width * .90, s.height * .39);
    path.cubicTo(s.width * .99, s.height * .35, s.width * 1.05, s.height * .48, s.width * 1.18, s.height * .42);
    ...
    // Motion is contained in that same single cloud.
    ...
    for (var i = 0; i < stars; i++) { final x = _noise(i * 2.13) * s.width, y = _noise(i * 4.71 + 3) * s.height, tw = .55 + .45 * math.sin(phase * math.pi * 2 * (1 + i % 3) + i), r = .22 + (i % 3) * .14; c.drawCircle(Offset(x, y), r, Paint()..color = Colors.white.withValues(alpha: (.018 + (i % 5) * .004) * tw)); }
