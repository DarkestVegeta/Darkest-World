...
class _Space extends CustomPainter {
  ...
  static void _preparePoints(Size size) { if (_pointSize == size && _points != null) return; _pointSize = size; _points = List.generate(_stars.length, (i) { final star = _stars[i]; return Offset(star.x * size.width, star.y * size.height); }, growable: false); }
  @override void paint(Canvas x, Size s) { _preparePoints(s); final points = _points!; for (final bucket in _buckets) { bucket.clear(); } final phaseStep = (phase.value * _twinkleSteps).floor() % _twinkleSteps; for (var i = 0; i < _stars.length; i++) { final star = _stars[i]; final paintIndex = _twinkleLut[(phaseStep + star.phaseIndex) % _twinkleSteps]; _buckets[paintIndex].add(points[i]); } for (var i = 0; i < _buckets.length; i++) { final bucket = _buckets[i]; if (bucket.isNotEmpty) x.drawPoints(PointMode.points, bucket, _starPaints[i]); } }
