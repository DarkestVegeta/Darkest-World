...
class SpacePainter extends CustomPainter {
  final double t; final bool safeMode;
  ...
  @override void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < count; i++) {
      final x = safeMode ? random.nextDouble() * size.width : (random.nextDouble() * size.width + t * 12) % size.width;
      final y = random.nextDouble() * size.height;
      canvas.drawCircle(Offset(x, y), .2 + random.nextDouble() * (safeMode ? .3 : .45), Paint()..color = const Color(0x18FFFFFF));
    }
  }
  @override bool shouldRepaint(covariant SpacePainter old) => old.t != t || old.safeMode != safeMode;
}

class OrbitView extends StatelessWidget {
  final List<GalaxyWorld> worlds; final double phase, zoom; final int? selected; final bool labels, detail, safeMode; final ValueChanged<int> onSelect;
  const OrbitView({super.key, required this.worlds, required this.phase, required this.selected, required this.labels, required this.zoom, required this.detail, required this.safeMode, required this.onSelect});
  @override Widget build(BuildContext context) => Stack(children: [
    Center(child: CustomPaint(size: const Size(700, 360), painter: OrbitPainter(detail: detail))),
    for (var i = 0; i < worlds.length; i++) Node(world: worlds[i], index: i, count: worlds.length, phase: phase, selected: selected == i, labels: labels, zoom: zoom, detail: detail, safeMode: safeMode, onTap: () => onSelect(i)),
  ]);
}

class Node extends StatelessWidget {
  final GalaxyWorld world; final int index, count; final double phase, zoom; final bool selected, labels, detail, safeMode; final VoidCallback onTap;
  const Node({super.key, required this.world, required this.index, required this.count, required this.phase, required this.selected, required this.labels, required this.zoom, required this.detail, required this.safeMode, required this.onTap});
  @override Widget build(BuildContext context) {
...
class OrbitPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) { final center = size.center(Offset.zero); final paint = Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = const Color(0x223F4A65); for (var i = 1; i <= (detail ? 4 : 2); i++) canvas.drawOval(Rect.fromCenter(center: center, width: 120.0 * i, height: 38.0 * i), paint); }
  @override bool shouldRepaint(covariant OrbitPainter old) => old.detail != detail;
}
