    ...
    return Stack(
      ...
      children: [
        for (final world in worlds.where((w) => positions.containsKey(w.kind)))
          Positioned(
            left: s.width * positions[world.kind]!.dx,
            top: s.height * positions[world.kind]!.dy,
          ...
          child: Column(
            ...
            children: [
              SizedBox(
                width: size,
                height: size,
                child: CustomPaint(painter: _WorldPlanetPainter(seed: world.kind.index + 21, accent: accent)),
              ),
...
class _WorldPlanetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      ...
      Paint()..shader = RadialGradient(
        colors: [accent.withValues(alpha: .30), accent.withValues(alpha: .07), Colors.transparent],
        stops: const [.0, .48, 1],
      ).createShader(Rect.fromCircle(center: c, radius: r * 1.24)),
    );
    canvas.drawCircle(
      c,
      r,
      Paint()..shader = const RadialGradient(
        center: Alignment(-.46, -.54), radius: 1.12,
        colors: [Color(0xFFB0A9AA), Color(0xFF6E6D78), Color(0xFF303441), Color(0xFF080A10)],
        stops: [.0, .30, .67, 1],
      ).createShader(sphere),
    );
    ...
    final landColors = [
      ...
      const Color(0xFFB29C78).withValues(alpha: .27),
      const Color(0xFF60766B).withValues(alpha: .30),
      const Color(0xFF484B59).withValues(alpha: .28),
    ];

    for (var i = 0; i < 4; i++) {
      final angle = i * 1.72 + rnd.nextDouble() * .45;
      final cx = c.dx + math.cos(angle) * r * .30;
      final cy = c.dy + math.sin(angle) * r * .27;
      final w = r * (.34 + rnd.nextDouble() * .25);
      final h = r * (.16 + rnd.nextDouble() * .16);
      ...
      for (var q = 1; q <= 3; q++) {
        final rr = 1 - q * .15;
        canvas.drawPath(
          _landmass(Offset(cx - r * .025 * q, cy + r * .018 * q), w * rr, h * rr, angle * .35, rnd, 15),
          Paint()..style = PaintingStyle.stroke..strokeWidth = .55..color = Colors.white.withValues(alpha: .035),
        );
      }
    }

    for (var i = 0; i < 42; i++) {
      final x = c.dx + (rnd.nextDouble() * 2 - 1) * r * .82;
      final y = c.dy + (rnd.nextDouble() * 2 - 1) * r * .72;
      canvas.drawCircle(
        Offset(x, y),
        r * (.002 + rnd.nextDouble() * .009),
        Paint()..color = Colors.white.withValues(alpha: .035 + rnd.nextDouble() * .045),
      );
    }
    canvas.restore();
    ...
    canvas.drawCircle(
      ...
      Paint()..shader = RadialGradient(
        ...
        stops: const [.42, 1],
      ).createShader(Rect.fromCircle(center: shadow, radius: r * .86)),
    );
    canvas.drawArc(
      Rect.fromCircle(center: c, radius: r * 1.005),
      math.pi * .60,
      math.pi * .88,
      false,
      Paint()..style = PaintingStyle.stroke..strokeWidth = 1.25..color = Colors.white.withValues(alpha: .22),
    );
    canvas.drawArc(
      Rect.fromCircle(center: c + Offset(-r * .035, -r * .035), radius: r * .93),
      math.pi * 1.02,
      math.pi * .42,
      false,
      Paint()..style = PaintingStyle.stroke..strokeWidth = r * .035..color = Colors.white.withValues(alpha: .035),
    );
  }
  ...
  Path _landmass(Offset c, double w, double h, double rot, math.Random rnd, int n) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      final wob = .78 + rnd.nextDouble() * .44;
      final x = math.cos(a) * w * .5 * wob;
      final y = math.sin(a) * h * .5 * wob;
      final xr = x * math.cos(rot) - y * math.sin(rot);
      final yr = x * math.sin(rot) + y * math.cos(rot);
      if (i == 0) {
        p.moveTo(c.dx + xr, c.dy + yr);
      } else {
        p.lineTo(c.dx + xr, c.dy + yr);
      }
    }
    p.close();
    return p;
