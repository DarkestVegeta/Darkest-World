...
class _DarkestWorldUniverseState extends State<DarkestWorldUniverse> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: const Color(0xFF010207), body: Stack(fit: StackFit.expand, children: [
      ...
      AnimatedBuilder(animation: clock, builder: (_, __) => Stack(fit: StackFit.expand, children: [
        CustomPaint(painter: _Space(clock.value)), CustomPaint(painter: _GalaxyVeil(clock.value)), CustomPaint(painter: _Orbital(clock.value)),
...
class _PlanetNode extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Positioned(left: p.dx - d / 2, top: p.dy - d / 2, width: d, height: d + 30, child: RepaintBoundary(child: GestureDetector(onTap: onTap, onDoubleTap: onOpen, child: Stack(children: [
...
class _PlanetPainter extends CustomPainter {
  ...
  static final Paint _glow = Paint(); static final Paint _body = Paint(); static final Paint _detail = Paint()..style = PaintingStyle.stroke..strokeWidth = .45..color = const Color(0x558FA2A9); static final Paint _rim = Paint()..style = PaintingStyle.stroke;
...
class _PlanetSelectionPainter extends CustomPainter {
  final double phase; const _PlanetSelectionPainter(this.phase); static final Paint _selection = Paint()..style = PaintingStyle.stroke..strokeWidth = 1.1..color = const Color(0xA0B7D0D8);
  @override void paint(Canvas x, Size s) { final c = Offset(s.width / 2, s.width / 2), r = s.width * .30; x.drawArc(Rect.fromCircle(center: c, radius: r * 1.32), phase * math.pi * 2, 1.4, false, _selection); }
...
class _GalaxyBackground extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    _background.shader = const RadialGradient(colors: [Color(0xFF0A141D), Color(0xFF02060B), Color(0xFF010207)]).createShader(Offset.zero & size);
    canvas.drawRect(Offset.zero & size, _background);
  }
  @override bool shouldRepaint(covariant _GalaxyBackground oldDelegate) => false;
}
...
class _Space extends CustomPainter {
  final double phase; const _Space(this.phase); static const int _twinkleSteps = 360; static final List<int> _twinkleLut = List.generate(_twinkleSteps, (step) => ((math.sin(step * math.pi * 2 / _twinkleSteps) + 1) * 4).round().clamp(0, 8)); static final List<_SpaceStar> _stars = List.generate(560, (i) { final r = math.Random(912 + i * 13); return _SpaceStar(r.nextDouble(), r.nextDouble(), r.nextDouble(), (i * 57) % _twinkleSteps); }); static final List<Paint> _starPaints = List.generate(9, (i) => Paint()..color = Colors.white.withValues(alpha: .018 + .075 * i / 8));
  @override void paint(Canvas x, Size s) { final phaseStep = (phase * _twinkleSteps).floor() % _twinkleSteps; for (var i = 0; i < _stars.length; i++) { final star = _stars[i]; final p = Offset(star.x * s.width, star.y * s.height); final paintIndex = _twinkleLut[(phaseStep + star.phaseIndex) % _twinkleSteps]; x.drawCircle(p, .18 + star.size * .62, _starPaints[paintIndex]); } }
  @override bool shouldRepaint(covariant _Space o) => o.phase != phase;
}

class _GalaxyVeil extends CustomPainter {
  final double phase; const _GalaxyVeil(this.phase); static final Paint _veil = Paint(); static final Paint _band = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round; static final Paint _inner = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
  @override void paint(Canvas canvas, Size size) { final short = math.min(size.width, size.height); final center = Offset(size.width * .5, size.height * .5); final rect = Rect.fromCenter(center: center, width: size.width * .78, height: short * .31); final drift = math.sin(phase * math.pi * 2) * .035; canvas.save(); canvas.translate(center.dx, center.dy); canvas.rotate(-.16 + drift); canvas.translate(-center.dx, -center.dy); _veil.shader = const RadialGradient(center: Alignment.center, radius: 1.0, colors: [Color(0x071E3850), Color(0x0A6C638B), Color(0x052B4962), Colors.transparent], stops: [.0, .34, .68, 1]).createShader(rect); canvas.drawOval(rect, _veil); _band.strokeWidth = math.max(1.0, short * .0022); _band.color = const Color(0x1A81759E); canvas.drawArc(rect, phase * math.pi * 2, 2.15, false, _band); final innerRect = Rect.fromCenter(center: center, width: size.width * .66, height: short * .18); _inner.strokeWidth = math.max(.55, short * .0011); _inner.color = const Color(0x126C8A9D); canvas.drawArc(innerRect, phase * math.pi * 2 + .9, 1.35, false, _inner); canvas.restore(); }
  @override bool shouldRepaint(covariant _GalaxyVeil oldDelegate) => oldDelegate.phase != phase;
}
