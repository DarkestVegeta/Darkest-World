...
class _IslandGameWorldPainter extends CustomPainter { final Animation<double> clock; _IslandGameWorldPainter(this.clock):super(repaint:clock);
  ...
  void drawIsland(Canvas c,Size s,List<Offset> pts,Color base,double scale,{bool main=false}){final path=island(s,pts),b=path.getBounds();c.drawPath(path.shift(Offset(0,11*scale)),Paint()..color=const Color(0x99000000));c.drawPath(path,Paint()..shader=LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[base.withValues(alpha:.99),base.withValues(alpha:.90),const Color(0xFF101823)]).createShader(b));final inner=Path()..addOval(Rect.fromCenter(center:b.center,width:b.width*.84,height:b.height*.76));c.drawPath(inner,Paint()..style=PaintingStyle.stroke..strokeWidth=main?2.0:1.15..color=const Color(0x3E9AA9A7));c.drawPath(path,Paint()..style=PaintingStyle.stroke..strokeWidth=1.25*scale..color=const Color(0x68758CA4));}
  void terrain(Canvas c,Size s,Rect area,int seed,{bool ridges=false}){final rng=math.Random(seed);final p=Paint()..style=PaintingStyle.stroke..strokeWidth=ridges?1.25:1..color=ridges?const Color(0x465F7389):const Color(0x305F7389);for(var i=0;i<(ridges?11:8);i++){final cx=area.left+area.width*(.12+rng.nextDouble()*.76),cy=area.top+area.height*(.16+rng.nextDouble()*.68);final rx=area.width*(.035+rng.nextDouble()*.13),ry=area.height*(.025+rng.nextDouble()*.11);c.drawOval(Rect.fromCenter(center:Offset(cx,cy),width:rx*2,height:ry*2),p);}}
  void plateau(Canvas c,Size s,Offset center,double w,double h,int seed){final r=Rect.fromCenter(center:center,width:w,height:h);c.drawOval(r.shift(const Offset(0,5)),Paint()..color=const Color(0x55000000));c.drawOval(r,Paint()..shader=RadialGradient(center:const Alignment(-.25,-.35),colors:[const Color(0xFF7B8780),const Color(0xFF4B5A58),const Color(0xFF29363A)]).createShader(r));terrain(c,s,r,seed,ridges:true);}
  @override void paint(Canvas canvas,Size s){final full=Offset.zero&s;canvas.drawRect(full,Paint()..shader=const RadialGradient(center:Alignment(.08,-.18),radius:1.0,colors:[Color(0xFF20354A),Color(0xFF0B1829),Color(0xFF020811)]).createShader(full));final water=Paint()..style=PaintingStyle.stroke..strokeWidth=1..color=const Color(0x1D477B9A);for(var i=0;i<8;i++){final rr=Rect.fromCenter(center:Offset(s.width*.52,s.height*.49),width:s.width*(.27+i*.105),height:s.height*(.17+i*.068));canvas.drawOval(rr,water);}
    final main=[const Offset(.15,.46),const Offset(.20,.34),const Offset(.32,.24),const Offset(.47,.20),const Offset(.61,.27),const Offset(.68,.40),const Offset(.64,.56),const Offset(.55,.67),const Offset(.39,.73),const Offset(.25,.66),const Offset(.17,.56)];drawIsland(canvas,s,main,const Color(0xFF596A68),1.0,main:true);drawIsland(canvas,s,[const Offset(.68,.13),const Offset(.78,.09),const Offset(.90,.15),const Offset(.88,.29),const Offset(.76,.33),const Offset(.68,.26)],const Color(0xFF465963),.74);drawIsland(canvas,s,[const Offset(.04,.25),const Offset(.13,.14),const Offset(.25,.19),const Offset(.28,.31),const Offset(.16,.38),const Offset(.07,.34)],const Color(0xFF4C5C61),.72);drawIsland(canvas,s,[const Offset(.69,.66),const Offset(.81,.59),const Offset(.93,.65),const Offset(.89,.79),const Offset(.76,.82),const Offset(.69,.75)],const Color(0xFF40515B),.70);drawIsland(canvas,s,[const Offset(.18,.75),const Offset(.29,.73),const Offset(.38,.81),const Offset(.31,.92),const Offset(.17,.86)],const Color(0xFF3D4D57),.66);
    plateau(canvas,s,Offset(s.width*.39,s.height*.39),s.width*.23,s.height*.15,8101);plateau(canvas,s,Offset(s.width*.52,s.height*.54),s.width*.18,s.height*.12,8102);terrain(canvas,s,Rect.fromLTWH(s.width*.19,s.height*.24,s.width*.47,s.height*.45),7301,ridges:true);terrain(canvas,s,Rect.fromLTWH(s.width*.68,s.height*.13,s.width*.23,s.height*.19),7302);terrain(canvas,s,Rect.fromLTWH(s.width*.70,s.height*.61,s.width*.23,s.height*.21),7303);
    final route=Paint()..style=PaintingStyle.stroke..strokeWidth=1.5..color=const Color(0x50869AA9);final path=Path()..moveTo(s.width*.27,s.height*.57)..quadraticBezierTo(s.width*.37,s.height*.49,s.width*.46,s.height*.54)..quadraticBezierTo(s.width*.55,s.height*.57,s.width*.60,s.height*.40);canvas.drawPath(path,route);final nodes=[const Offset(.29,.52),const Offset(.46,.54),const Offset(.60,.40),const Offset(.79,.22),const Offset(.16,.28),const Offset(.81,.70)];final node=Paint()..color=const Color(0x9A9BAFC5);for(var i=0;i<nodes.length;i++){final q=Offset(nodes[i].dx*s.width,nodes[i].dy*s.height);final pulse=.82+.20*math.sin(clock.value*math.pi*2+i);canvas.drawCircle(q,2.35*pulse,node);}
    canvas.drawRect(full,Paint()..shader=const RadialGradient(center:Alignment(0,-.10),radius:1.0,colors:[Colors.transparent,Color(0x18020A14),Color(0x60020812)]).createShader(full));final title=TextPainter(text:TextSpan(text:'GAME WORLD',style:TextStyle(color:Colors.white.withValues(alpha:.72),fontSize:math.max(13,s.width*.018),letterSpacing:4.2,fontWeight:FontWeight.w500)),textDirection:TextDirection.ltr)..layout();title.paint(canvas,Offset((s.width-title.width)/2,s.height*.94));}
