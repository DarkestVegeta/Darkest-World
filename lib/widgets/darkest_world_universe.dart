...
class _DarkestWorldUniverseState extends State<DarkestWorldUniverse> with SingleTickerProviderStateMixin {
  ...
  void tap(int i) {
    setState(() => focused = focused == i ? null : i);
    GalaxyNavigationSession.instance.selected = widget.worlds[i].kind.name;
    widget.onWorldTap?.call(widget.worlds[i]);
  }
...
class _PlanetNode extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final c = diameter / 2;
    final orbit = diameter * (.20 + (index % 4) * .075);
...
class _PlanetPainter extends CustomPainter {
  ...
  @override void paint(Canvas x, Size s) {
    ...
    _glow.shader = RadialGradient(colors: [const Color(0x5592B4C1).withValues(alpha: selected ? .25 : .07), Colors.transparent]).createShader(Rect.fromCircle(center: c, radius: r * 1.8));
    x.drawCircle(c, r * 1.8, _glow);
    _body.shader = RadialGradient(center: const Alignment(-.42, -.5), colors: [base, base.withValues(alpha: .78), const Color(0xFF26313A), const Color(0xFF02050A)]).createShader(bounds);
...
class _GalaxyVeil extends CustomPainter {
  final double phase; const _GalaxyVeil(this.phase);
  static final Paint _veil = Paint(); static final Paint _band = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round; static final Paint _inner = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
  @override void paint(Canvas canvas, Size size) {
    final short = math.min(size.width, size.height); final center = Offset(size.width * .5, size.height * .5); final rect = Rect.fromCenter(center: center, width: size.width * .78, height: short * .31); final drift = math.sin(phase * math.pi * 2) * .035;
    canvas.save(); canvas.translate(center.dx, center.dy); canvas.rotate(-.16 + drift); canvas.translate(-center.dx, -center.dy);
    _veil.shader = const RadialGradient(center: Alignment.center, radius: 1.0, colors: [Color(0x071E3850), Color(0x0A6C638B), Color(0x052B4962), Colors.transparent], stops: [.0, .34, .68, 1]).createShader(rect); canvas.drawOval(rect, _veil);
    _band.strokeWidth = math.max(1.0, short * .0022); _band.color = const Color(0x1A81759E); canvas.drawArc(rect, phase * math.pi * 2, 2.15, false, _band);
    final innerRect = Rect.fromCenter(center: center, width: size.width * .66, height: short * .18); _inner.strokeWidth = math.max(.55, short * .0011); _inner.color = const Color(0x126C8A9D); canvas.drawArc(innerRect, phase * math.pi * 2 + .9, 1.35, false, _inner); canvas.restore();
  }
  @override bool shouldRepaint(covariant _GalaxyVeil oldDelegate) => oldDelegate.phase != phase;
}

class _Orbital extends CustomPainter {
  final double phase; const _Orbital(this.phase);
  static final Paint _rings = Paint()..style = PaintingStyle.stroke..strokeWidth = .45..color = const Color(0x1C8BA5AF); static final Paint _arc = Paint()..style = PaintingStyle.stroke..strokeWidth = 1.0..color = const Color(0x527F9BA7);
  @override void paint(Canvas x, Size s) { final c = Offset(s.width * .5, s.height * .5); for (var i = 0; i < 5; i++) { final w = s.width * (.42 + i * .10); final h = s.height * (.18 + i * .04); x.drawOval(Rect.fromCenter(center: c, width: w, height: h), _rings); } x.drawArc(Rect.fromCenter(center: c, width: s.width * .72, height: s.height * .40), phase * math.pi * 2, .75, false, _arc); }
  @override bool shouldRepaint(covariant _Orbital o) => o.phase != phase;
}
