...
class _UniverseState extends State<DarkestWorldUniverse> with SingleTickerProviderStateMixin {
  ...
  void select(int index) {
    ...
    final next = selected == index ? null : index;
    setState(() => selected = next);
    if (next != null) widget.onWorldTap?.call(widget.worlds[index]);
  }

  void move(int delta) {
    if (widget.worlds.isEmpty) return;
    final current = selected ?? 0;
    select((current + delta + widget.worlds.length) % widget.worlds.length);
  }
  ...
  KeyEventResult handleKey(FocusNode node, KeyEvent event) {
    ...
    if (digit >= 0) { select(digit); return KeyEventResult.handled; }
    if (key == LogicalKeyboardKey.arrowLeft) { move(-1); return KeyEventResult.handled; }
    if (key == LogicalKeyboardKey.arrowRight) { move(1); return KeyEventResult.handled; }
    if (key == LogicalKeyboardKey.enter) { visitSelected(); return KeyEventResult.handled; }
    if (key == LogicalKeyboardKey.keyM) { setState(() => map = !map); return KeyEventResult.handled; }
    if (key == LogicalKeyboardKey.keyL) { setState(() => labels = !labels); return KeyEventResult.handled; }
            ...
            builder: (_, __) {
              ...
              return Stack(
                ...
                children: [
                  ...
                  else OrbitView(worlds: widget.worlds, phase: phase, selected: selected, labels: labels, zoom: zoom, onSelect: select),
                  ...
                  if (selected != null) Positioned(right: 18, bottom: 58, child: WorldPanel(world: widget.worlds[selected!], onEnter: visitSelected, onClose: () => setState(() => selected = null))),
                  ...
                  Positioned(right: 18, bottom: 16, child: Row(children: [
                    ...
                    const SizedBox(width: 5), Control(label: 'LABELS', onTap: () => setState(() => labels = !labels)),
                    const SizedBox(width: 5), Control(label: 'DETAIL', onTap: () {}),
                    const SizedBox(width: 5), Control(label: 'SAFE', onTap: () {}),
                    ...
                    const SizedBox(width: 5), Control(label: 'RESET', onTap: () => setState(() { zoom = 1; manualPhase = 0; selected = null; })),
...
class OrbitView extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Stack(children: [
    Center(child: CustomPaint(size: const Size(700, 360), painter: OrbitPainter())),
    for (var i = 0; i < worlds.length; i++) Node(world: worlds[i], index: i, count: worlds.length, phase: phase, selected: selected == i, labels: labels, zoom: zoom, onTap: () => onSelect(i)),
  ]);
}
...
class Node extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    final x = .5 + math.cos(angle) * .28; final y = .5 + math.sin(angle) * .20;
    final depth = (.5 + math.sin(angle) * .5).clamp(0.0, 1.0); final size = (38 + depth * 22 + (selected ? 18 : 0)) * zoom;
    return Align(alignment: Alignment(x * 2 - 1, y * 2 - 1), child: Semantics(label: world.title, button: true, selected: selected, child: GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (labels) Text(world.title.toUpperCase(), style: TextStyle(color: selected ? Colors.white : const Color(0x99FFFFFF), fontSize: selected ? 8 : 6, letterSpacing: 1.3)),
    ]))));
  }
}
...
class OrbitPainter extends CustomPainter {
  const OrbitPainter();
  @override void paint(Canvas canvas, Size size) { final center = size.center(Offset.zero); final paint = Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = const Color(0x223F4A65); for (var i = 1; i <= 4; i++) canvas.drawOval(Rect.fromCenter(center: center, width: 120.0 * i, height: 38.0 * i), paint); }
...
class WorldPanel extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(width: 300, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xE8070910), border: Border.all(color: const Color(0x44FFFFFF))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Row(children: [Expanded(child: Control(label: 'PREVIOUS', onTap: () {})), const SizedBox(width: 4), Expanded(child: Control(label: 'CURRENT', onTap: () {})), const SizedBox(width: 4), Expanded(child: Control(label: 'NEXT', onTap: onEnter))]),
    const SizedBox(height: 7), Row(children: [Expanded(child: Control(label: 'VISIT', onTap: onEnter)), const SizedBox(width: 4), Expanded(child: Control(label: 'CLOSE', onTap: onClose))]),
  ]));
}
