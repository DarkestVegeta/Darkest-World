...
class _DarkestWorldUniverseState extends State<DarkestWorldUniverse> {
  ...
  Widget build(BuildContext context) {
    ...
    final current = selected == null ? null : widget.worlds.where((world) => world.kind == selected).firstOrNull;
    return Scaffold(
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawRect(
      ...
      Paint()..shader = const RadialGradient(
        center: Alignment(0, -.1), radius: 1.15,
...
class _GalaxyPlanets extends StatelessWidget {
  ...
  static const pos = <GalaxyWorldKind, Offset>{
    GalaxyWorldKind.vegeta: Offset(.50, .53), GalaxyWorldKind.game: Offset(.78, .25),
    GalaxyWorldKind.identity: Offset(.20, .34), GalaxyWorldKind.cinema: Offset(.84, .59),
    GalaxyWorldKind.creation: Offset(.22, .73), GalaxyWorldKind.music: Offset(.61, .81),
    GalaxyWorldKind.family: Offset(.39, .17), GalaxyWorldKind.archive: Offset(.09, .56),
    GalaxyWorldKind.comingSoon: Offset(.91, .80),
  };
  static const fac = <GalaxyWorldKind, double>{
    GalaxyWorldKind.vegeta: .34, GalaxyWorldKind.game: .205, GalaxyWorldKind.identity: .175,
    GalaxyWorldKind.cinema: .165, GalaxyWorldKind.creation: .155, GalaxyWorldKind.music: .14,
    GalaxyWorldKind.family: .125, GalaxyWorldKind.archive: .095, GalaxyWorldKind.comingSoon: .08,
  };
...
class _Planet extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final accent = world.kind == GalaxyWorldKind.vegeta ? const Color(0xFF9A7BC5) : const Color(0xFF657BA7);
        ...
        child: SizedBox(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(
              ...
              style: TextStyle(color: Colors.white.withValues(alpha: .86), fontSize: world.kind == GalaxyWorldKind.vegeta ? 11 : 8, letterSpacing: world.kind == GalaxyWorldKind.vegeta ? 3 : 1.7),
            ),
          ]),
        ),
      ),
...
class _WorldPlanetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      ...
      radius,
      Paint()..shader = const RadialGradient(
        center: Alignment(-.38, -.46), radius: 1.08,
    ...
    for (var i = 0; i < 18; i++) {
      final p = Offset(center.dx + (random.nextDouble() * 2 - 1) * radius * .72, center.dy + (random.nextDouble() * 2 - 1) * radius * .66);
      canvas.drawCircle(p, radius * (.008 + random.nextDouble() * .035), Paint()..color = Colors.white.withValues(alpha: .025));
    }
    canvas.restore();
    ...
    canvas.drawCircle(
      ...
      radius * .82,
      Paint()..shader = RadialGradient(colors: [Colors.transparent, Colors.black.withValues(alpha: .56)])
          .createShader(Rect.fromCircle(center: center + Offset(radius * .40, radius * .10), radius: radius * .82)),
    );
    canvas.drawArc(
      Rect.fromCircle(center: center, radius: radius * 1.012), math.pi * .66, math.pi * .78, false,
      Paint()..style = PaintingStyle.stroke..strokeWidth = 1.1..color = Colors.white.withValues(alpha: .22),
    );
  }
...
class _SelectionPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Material(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(color: const Color(0xE6090914), border: Border.all(color: Colors.white.withValues(alpha: .12)), borderRadius: BorderRadius.circular(18)),
