...
class _ArchiveAtmospherePainter extends CustomPainter {
  ...
  void _ensureGeometry(Size s){
    ...
    _cachedSize=s;_short=math.min(s.width,s.height);
    final center=Offset(s.width*.5,s.height*.46);
    _fullRect=Offset.zero&s;
    _hazeRect=Rect.fromCenter(center:Offset(center.dx,center.dy+_short*.06),width:s.width*.92,height:_short*.30);
    _arcRect=Rect.fromCenter(center:center,width:_short*.76,height:_short*.235);
    _core=_short*.13;
    ...
    _coreShader=RadialGradient(colors:[const Color(0x4B8E86AE),const Color(0x0A8C7EA0),Colors.transparent]).createShader(_coreRect);
    _geometryReady=true;
  }
