...
class _DarkestWorldArtboxState extends State<DarkestWorldArtbox> {
  ...
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, box) {
    ...
    final px = (_pointer.dx / math.max(1, w) - .5).clamp(-.5, .5);
    final py = (_pointer.dy / math.max(1, h) - .5).clamp(-.5, .5);
    final tiltX = py * -.065;
    final tiltY = px * .085 + math.sin(widget.phase * math.pi * 2) * .009;
    return MouseRegion(
      ...
      child: Stack(alignment: Alignment.center, children: [
        ...
        Transform(alignment:Alignment.center,transform:Matrix4.identity()..setEntry(3,2,.0013)..rotateX(tiltX)..rotateY(tiltY),child:Stack(alignment:Alignment.center,children:[
          Container(width:w*(widget.compact?.64:.58),height:h*.78,decoration:BoxDecoration(color:const Color(0x0EFFFFFF),borderRadius:BorderRadius.circular(6),border:Border.all(color:const Color(0x287F70B0)),boxShadow:[BoxShadow(color:const Color(0xFF3F315F).withValues(alpha:.25),blurRadius:42,spreadRadius:3)])),
          SizedBox(width:w*(widget.compact?.73:.67),height:h*.82,child:_image()),
        ...
        Positioned(top:10,left:10,child:_badge('ARTBOX / 3D')), Positioned(top:10,right:10,child:_badge('TRANSPARENT')),
  ...
  Widget _image()=>widget.imageUrl==null?const SizedBox.shrink():Image.network(widget.imageUrl!,fit:BoxFit.contain,filterQuality:FilterQuality.high,errorBuilder:(_,__,___)=>Center(child:Text(widget.title.toUpperCase(),textAlign:TextAlign.center,style:const TextStyle(fontSize:11,letterSpacing:1.5,color:Color(0x557F8AA2)))));
...
class _ArtboxEnvironmentPainter extends CustomPainter {
  final double phase; const _ArtboxEnvironmentPainter(this.phase);
  @override void paint(Canvas c,Size s){
    ...
    final sweep=(phase*math.pi*2)%(math.pi*2);c.drawArc(Rect.fromCenter(center:center,width:s.width*.84,height:s.width*.26),sweep,.58,false,Paint()..style=PaintingStyle.stroke..strokeWidth=1.15..color=const Color(0x5A8C82A5));
    final rnd=math.Random(2246);for(var i=0;i<260;i++){final x=rnd.nextDouble()*s.width,y=rnd.nextDouble()*s.height,d=.2+rnd.nextDouble();c.drawCircle(Offset(x,y),.2+d*.75,Paint()..color=Colors.white.withValues(alpha:.018+d*.12));}
    final scan=(phase*s.height*1.2)%(s.height+100)-50;c.drawRect(Rect.fromLTWH(0,scan,s.width,1),Paint()..color=const Color(0x127F70B0));
