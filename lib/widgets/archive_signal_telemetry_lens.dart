  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: active ? [BoxShadow(color: const Color(0x247F70B0), blurRadius: 10 + pulse * 6)] : null,
      ),
      child: Icon(
        Icons.adjust,
        size: 14,
        color: active ? Color.lerp(const Color(0x779A8AC4), const Color(0xCCB2A5D8), pulse) : const Color(0x556F7890),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    if (state.related.isEmpty) {
      return Container(
        ...
        child: const Text('NO RELATED SIGNALS', style: TextStyle(fontSize: 5.5, letterSpacing: 1.4, color: Color(0x557F8AA2))),
      );
    }
          ...
          child: Material(
            ...
            child: InkWell(
              ...
              child: Container(
                height: compact ? 22 : 25,
                padding: const EdgeInsets.symmetric(horizontal: 7),
                decoration: BoxDecoration(color: const Color(0x0C7F70B0), border: Border.all(color: Color.lerp(const Color(0x147F70B0), const Color(0x397F70B0), pulse)!)),
                child: Row(children: [
                  Container(width: 3, height: 3, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0x667F70B0), boxShadow: [BoxShadow(color: const Color(0x247F70B0), blurRadius: 4 + pulse * 3)])),
                  const SizedBox(width: 6),
                  Expanded(child: Text(_label(visible[i].title), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 5.2, letterSpacing: 1.0, color: Color(0x667F8AA2)))),
                ]),
              ),
            ),
          ),
...
class _ArchiveSignalPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final ring = Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = const Color(0x227F70B0);
    canvas.drawCircle(center, radius, ring);
    canvas.drawCircle(center, radius * .62, ring);
    final sweepPaint = Paint()..style = PaintingStyle.stroke..strokeWidth = 1.2..color = const Color(0x3C9A8AC4);
    ...
    if (relatedCount > 0) {
      final relatedPaint = Paint()..style = PaintingStyle.stroke..strokeWidth = .7..color = const Color(0x187F70B0);
  ...
  @override
  bool shouldRepaint(covariant _ArchiveSignalPainter oldDelegate) => oldDelegate.phase != phase || oldDelegate.links != links || oldDelegate.related != related;
}
